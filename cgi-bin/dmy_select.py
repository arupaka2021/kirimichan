...
<html lang="ja">
  ...
  <h2>現在日時：{now}</h2>
  <table>
    <tr><th>ID</th><th>氏名</th><th>カレーの食べ方</th></tr>
'''[1:-1].format(css=css_array[array_num],title=title_str,now=now_str))

# データベース接続部分
db = pypg.dmy()
sql = "SELECT id, lname||fname AS fullname, curry FROM dmy ORDER BY id LIMIT 5"
...
for r in rows:
    print('    <tr><td>{}</td><td>{}</td><td>{}</td></tr>'.format(r['id'], r['fullname'], r['curry']))
